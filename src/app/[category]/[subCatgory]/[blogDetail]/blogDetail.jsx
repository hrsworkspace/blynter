...
const BlogDetailPage = ({ category, blogDetails, relatedArticles = [] }) => {
  ...
  if (!blogDetails) {
    return (
      <main className="min-h-screen bg-[#f9f9f7] dark:bg-gray-900 flex items-center justify-center">
      ...
      {blogDetails && (
        <>
          ...
          <article className="max-w-4xl mx-auto px-4 py-12">
            ...
            {/* Back Button */}
            {blogDetails?.faqs?.length > 0 && <section className="max-w-3xl mx-auto px-4 py-12">
              {/* Section Title */}
              ...
              {/* FAQ Items */}
              <div className="space-y-4">
                {blogDetails?.faqs?.map((faq, index) => (
                  <div
                    key={index}
                    className="border border-gray-200 dark:border-gray-800 rounded-2xl shadow-sm bg-white dark:bg-gray-900 transition-all duration-300"
                  >
                    {/* Question */}
                    <button
            ...
            <nav
              className="flex max-sm:flex-col max-sm:gap-8 max-sm:items-start justify-between items-center mt-5 pt-8 border-t border-black/20 dark:border-gray-500"
